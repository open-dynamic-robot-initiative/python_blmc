...
def motor_positions_to_joint_angles(mpos1, mpos2):
    return (
        mpos1 * 2 * np.pi / LegProp.joint_1_gear_factor,
...
def joint_angles_to_motor_position(th1, th2):
    return (
        th1 / (2 * np.pi) * LegProp.joint_1_gear_factor,
...
class Transformer:
    ...
    def tf01(self):
        ...
        Sth = np.sin(self.th1)
        return np.array(
            [[Cth, Sth, LegProp.l1 * Cth], [Sth, -Cth, LegProp.l1 * Sth], [0, 0, 1]]
        )
    ...
    def tf12(self):
        ...
        Sth = np.sin(self.th2)
        return np.array(
            [[Cth, -Sth, LegProp.l2 * Cth], [Sth, Cth, LegProp.l2 * Sth], [0, 0, 1]]
        )
    ...
    def foot_position(self):
        ...
        if self._foot_pos is None:
            self._foot_pos = self.tf01().dot(self.tf12().dot(np.array([0, 0, 1])))[:2]
    ...
    def is_pose_safe(self):
        ...
        """
        return (
            (abs(self._mtr_data.mtr1.position.value) < 1.2)
    ...
    def foot_force(self, iq1, iq2):
        ...
        fx = -(
            ...
            * (l2 * t1 * sin(q1 - q2) + t2 * (l1 * sin(q1) + l2 * sin(q1 - q2)))
        ) / (l1 * l2 * (l1 * sin(q1) + l2 * sin(q1 - q2)) * sin(q2))
        fy = -(l1 * t2 * sin(q1) + l2 * t1 * sin(q1 - q2) + l2 * t2 * sin(q1 - q2)) / (
            l1 * l2 * sin(q2)
        )
